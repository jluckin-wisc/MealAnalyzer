...
//System.out.println("Inserting into Internal Node: " + key);
        	if(value == null) {
        		...
        		for(int i=0; i<this.keys.size();i++) {
        			if(key.compareTo(this.keys.get(i))<=0){
        				...
        				//If i is equal to keys.size() then the key is lareger
    ...
    private class LeafNode extends Node {
        ...
        List<V> rangeSearch(K key, String comparator) {
        	...
        	ArrayList<K> keyList = new ArrayList<K>(this.keys);
        	ArrayList<V> valuesList = new ArrayList<V>(this.values);
        		...
        		while(!finished) {
        			for(int i=0; i<keyList.size();i++) {
        				if(keyList.get(i).compareTo(key)<=0) {
        					results.add(valuesList.get(i));
        				}else if (keyList.get(i).compareTo(key)>0){
        					//Returning list
        					return results;
        				}
        			}
        			...
        			if(current.next!=null) { //Continues if next isnt null
        				...
        				keyList = new ArrayList<K>(current.keys);
        				valuesList = new ArrayList<V>(current.values);
        			}	else {
        					return results;
        				}
        		}
